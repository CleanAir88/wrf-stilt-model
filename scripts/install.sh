...
apt-get install -y build-essential csh gfortran m4 curl perl ${mpirun_packages} libpng-dev netcdf-bin libnetcdff-dev ${extra_packages}
...
# wget https://github.com/wrf-model/WPS/archive/refs/tags/v${WPSversion}.tar.gz -O WPSV${WPSversion}.tar.gz
...
#########################################
#	OBSGRID Installation		#
#########################################
apt-get install gcc-9 gfortran-9 -y

# 调整gcc-12的优先级为40，gcc-9的优先级为100
update-alternatives --install /usr/bin/gcc gcc /usr/bin/gcc-12 40
update-alternatives --install /usr/bin/gcc gcc /usr/bin/gcc-9 100
# 调整gfortran-12的优先级为40，gfortran-9的优先级为100
update-alternatives --install /usr/bin/gfortran gfortran /usr/bin/gfortran-12 40
update-alternatives --install /usr/bin/gfortran gfortran /usr/bin/gfortran-9 100

tar -zxvf OBSGRID.tar.gz
cd OBSGRID
echo 2 | ./configure
sed -i "s#NETCDF_LIBS.*#NETCDF_LIBS     =       -L\${NETCDF}/lib -lnetcdff -lnetcdf#" configure.oa
sed -i "s#CPP.*/usr/bin/cpp -C -P -traditional#CPP             =       /usr/bin/cpp -P -traditional#" configure.oa
./compile

# 编译get_rda_data.f
cd util
gfortran -ffree-form get_rda_data.f -o get_rda_data.exe


echo "install STILT start"
# 此为注册版源码，请到https://www.ready.noaa.gov/HYSPLIT_register.php申请
# cp -r bin/linux-gnu/* ${STILT_WD}/exe/
chmod +x ${STILT_WD}/setup
chmod +x ${STILT_WD}/exe/*
R -e "install.packages(c('dplyr', 'ncdf4', 'parallel', 'rslurm', 'raster', 'R.utils'), repos='https://mirrors.tuna.tsinghua.edu.cn/CRAN/')"
cd ${STILT_WD}
./setup
Rscript r/dependencies.r
echo "STILT installation completed!"

echo "install mmif start"
cd /src/mmif
unzip MMIFv4.1.zip
cp makefile MMIF4.1/
cd MMIF4.1
make
echo "mmif installation completed!"
